...
$(function(){if('createCode'== locat[3]){locat =  locat[0]+'//'+locat[2];}else{locat =  locat[0]+'//'+locat[2]+'/'+locat[3];};});
	...
	function addWorkflowProp(){
		
		var dname ="userId"; 	 		 //属性名
		var dtype ="String"; 	 		 //类型
		var dbz	  = "用户";   	 		 //备注
		var isQian = "是"; 		 //是否前台录入
		var ddefault ="无"; 	 //默认值
		var msgIndex = ""; 	 //msgIndex不为空时是修改
		var flength = "50"; 	 	 //长度
		var decimal = ""; 	 	 //小数
	    ...
	    if(!isSame(dname)){
	      ...
	      numArray.push(arrayField(fields));
		  ...
		  isQian = "否"; 		 //是否前台录入
		  ddefault ="无"; 	 //默认值
		  msgIndex = ""; 	 //msgIndex不为空时是修改
		  flength = "50"; 	 	 //长度
		  decimal = ""; 	 	 //小数
		  dname = dname.toUpperCase();
		  var fields2 = dname + ',fh,' + dtype + ',fh,' + dbz + ',fh,' + isQian + ',fh,' + ddefault + ',fh,' + flength + ',fh,' + decimal;
	      numArray.push(arrayField(fields2));
		 

	      dname ="processInstanceId"; 	 		 //属性名
		  dtype ="String"; 	 		 //类型
		  dbz	 = "流程实例Id";   	 		 //备注
		  isQian = "否"; 		 //是否前台录入
		  ddefault ="无"; 	 //默认值
		  ...
		  decimal = ""; 	 	 //小数
		  dname = dname.toUpperCase();
		  var fields3 = dname + ',fh,' + dtype + ',fh,' + dbz + ',fh,' + isQian + ',fh,' + ddefault + ',fh,' + flength + ',fh,' + decimal;
	      numArray.push(arrayField(fields3));
	 
	}
	
	function deleteflowField(){
		removeField(numArray[2]+"");
		removeField(numArray[1]+"");
		removeField(numArray[0]+"");
