										...
										<#list fieldList as var>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<c:if test='${r"${var."}STATE == "未提交" }'>
														<a class="btn btn-xs btn-warning" onclick="startApply('${r"${var."}${objectNameUpper}_ID${r"}"}');"  title="检索">
														提交申请</a>
													 ...
													 <c:if test='${r"${var."}STATE != "未提交" }'>
													 	<a class="btn btn-xs btn-success" title="编辑" onclick="openListComment('${r"${var."}PROCESSINSTANCEID${r"}"}');">
															<i class="ace-icon fa fa-search bigger-110 nav-search-icon blue" title="编辑"></i>查看历史批注
														</a>
	...
	<script type="text/javascript">
		...
		function startApply(Id){
			...
			bootbox.confirm("确定要提交申请吗?", function(result) {
					if(result) {
						top.jzts();
						var url = "<%=basePath%>${objectNameLower}/startApply.do?${objectNameUpper}_ID="+Id+"&tm="+new Date().getTime();
