...
<#list fieldList as var>
	...
	public void startApply(PrintWriter out) throws Exception{
		...
		logBefore(logger, Jurisdiction.getUsername()+"提交${objectName}流程");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		String ${objectNameLower}_ID=pd.getString("${objectNameUpper}_ID"); 
		Map<String,Object> variables=new HashMap<String,Object>();
		variables.put("${objectNameLower}_ID", ${objectNameLower}_ID);
		// 启动流程
	...
	public ModelAndView get${objectNameLower}ByTaskId(PrintWriter out) throws Exception{		
		...
		List<PageData>	varList =${objectNameLower}Service.listAll(pageData);
		mv.setViewName("${packageName}/${objectNameLower}/task${objectNameLower}_list");
		mv.addObject("varList", varList);
		mv.addObject("pd", pd);
		mv.addObject("QX",Jurisdiction.getHC());	//按钮权限
