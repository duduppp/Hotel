						...
						<div class="col-xs-12">
								<form action="user/${msg }.do" name="userForm" id="userForm" method="post">
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										<c:if test="${fx != 'head'}">
										<tr>
											<td style="width:79px;text-align: right;padding-top: 13px;"><font style="color: red">*</font>角色:${fx}</td>
											<td id="juese">
											 <select id="roleselect_id"  >
												<c:forEach items="${roleList}" var="role">
													<option value="${role.ROLE_ID }" <c:if test="${role.ROLE_ID == pd.ROLE_ID }">selected</c:if>>${role.ROLE_NAME }</option>
												</c:forEach>
											 </select>
											 <input type="hidden" id="role_id" name="ROLE_ID" value="${pd.ROLE_ID}"/>
											  <input type="hidden" id="roleName" name="roleName" value="${pd.ROLE_REMARK }"/>
											</td>
										</tr>
										</c:if>
										<tr>
											<td style="width:89px;text-align: right;padding-top: 13px;">
											<font style="color: red">*</font>用户名:</td>
										...
										<tr>
											<td style="width:79px;text-align: right;padding-top: 13px;"><font style="color: red">*</font>密码:</td>
											<td><input type="password" name="PASSWORD" id="password"  maxlength="32" placeholder="输入密码" title="密码" style="width:98%;"/></td>
										</tr>
										<tr>
											<td style="width:79px;text-align: right;padding-top: 13px;"><font style="color: red">*</font>确认密码:</td>
											<td><input type="password" name="chkpwd" id="chkpwd"  maxlength="32" placeholder="确认密码" title="确认密码" style="width:98%;"/></td>
										</tr>
										<tr>
											<td style="width:79px;text-align: right;padding-top: 13px;"><font style="color: red">*</font>姓名:</td>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#roleselect_id').multiselect({
		     nonSelectedText: '请选择',
            enableFiltering: true,  
            minmumCountSelect:6,
            buttonWidth: '300px',
            onChange: function (option, checked) {//change事件改变   
            	var rolevalues=$(".multiselect-selected-text").parent().attr("title");
            	var reg = new RegExp(",","g");//g,表示全部替换。
            	rolevalues=rolevalues.replace(reg,"@dp");
            	$("#roleName").val(rolevalues); 
            }
		});
	...
	function save(){
		...
		var selectValue=$("#roleselect_id").val()+'';
		
		var reg = new RegExp(",","g");//g,表示全部替换。
		selectValue=selectValue.replace(reg,"@dp");
		
	 
		
		
		$("#role_id").val(selectValue);  
		if($("#role_id").val()==""){
			$("#juese").tips({
				side:3,
	            msg:'选择角色',
	...
	$(function() {  
		...
		var roleArr=roleId.split("@dp");
		
		for(var i in roleArr){
			
			$('#roleselect_id').multiselect('select', roleArr[i]);
		}
