							...
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;">
								  ...
								  <select id="roleselect_id"  >
											  ...
											  <input type="hidden" id="ROLE_REMARK" name="ROLE_REMARK" value="${pd.ROLE_REMARK }"/>
								</td>
								<c:if test="${QX.cha == 1 }">
								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										...
										<tr>
											...
											<td class="center">${fn:replace(user.ROLE_REMARK,'@dp', ',')}</td>
...
<!-- 											<i class="ace-icon fa fa-envelope-o"></i></a></td> -->
		...
		$('#roleselect_id').multiselect({
	         ...
	         if(rolevalues!="请选择"){
	          	 var reg = new RegExp(",","g");//g,表示全部替换。
	 	       	...
	 	       	$("#ROLE_REMARK").val(rolevalues);  
	         }
	       }
		});
		 
		if(roleNames.length>0){
			var roleArr=roleNames.split("@dp");
			for(var i in roleArr){
				var roleName=$.trim(roleArr[i]);
				 $('#roleselect_id').multiselect('select', roleName);
			}
		} 
...
function toExcel(){
	...
	window.location.href='<%=basePath%>user/excel.do?keywords='+keywords+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&ROLE_REMARK='+ROLE_REMARK;
}
