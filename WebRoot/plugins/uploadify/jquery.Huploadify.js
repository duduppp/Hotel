...
$.fn.Huploadify = function(opts){
	var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-progress">'+
		'<div class="uploadify-progress-bar"></div></div><span class="up_filename">${fileName}</span>'+
		'<span class="uploadbtn">上传</span><span class="delfilebtn">删除</span></div>';
	var defaults = {
	...
	this.each(function(){
		...
		var inputStr = '<input id="select_btn_'+instanceNumber+'"  capture="camera" class="selectbtn" style="display:none;" type="file" name="fileNmae"';
			  ...
			  if (xhr.upload) {
				  ...
				  if(option.formData){
				  	for(key in option.formData){ 
				  		console.info(key);
				  		console.info('----------------------------'); 
				  		console.info(option.formData[key]);
