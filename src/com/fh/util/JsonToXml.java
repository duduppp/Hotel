...
public class JsonToXml {
		...
		while (it.hasNext()) {
			...
			if (hasChild(jsonObj, (String) m.getKey())) {
				if (hasAttribute(jsonObj, (String) m.getKey())) {
					...
					sb.append(getXML(
							jsonObj.getJSONObject((String) m.getKey()),
							nDepth + 1));
					sb.append("</");
					sb.append(m.getKey());
					sb.append(">\n");
				...
				if (((String) m.getKey()).equals("CDATA")) {
					...
					if (jsonObj.get(m.getKey()).getClass()
							.equals(new JSONArray().getClass())) {
						sb.append(getJsonArrayData(
								(JSONArray) jsonObj.get(m.getKey()),
								(String) m.getKey()));
					} else {
						sb.append("<");
						sb.append(m.getKey());
						...
						sb.append(reservedReplace(String.valueOf(jsonObj.get(m
								.getKey()))));
						sb.append("</");
						sb.append(m.getKey());
						sb.append(">\n");
	...
	private static String getAttribute(JSONObject json) {
		...
		while (it.hasNext()) {
			...
			if (((String) m.getKey()).substring(0, 1).equals("@")) {
				...
				sb.append("=");
				append(sb,
						reservedReplace(String.valueOf(json.get(m.getKey()))));
	...
	private static String getText(JSONObject jsonObj) {
		...
		while (it.hasNext()) {
			Map.Entry m = (Map.Entry) it.next();
			if (((String) m.getKey()).substring(0, 1).equals("#")) {
				strData = reservedReplace(String
						.valueOf(jsonObj.get(m.getKey())));
