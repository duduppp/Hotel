...
public class XmlToJson {
	...
	public static String getCdataString(String value) {
		return new JSONStringer().object().key("CDATA").value(value)
				.endObject().toString();
	}
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (element.hasAttributes()) {
					...
					for (int j = 0; j < map.getLength(); j++) {
						sb.append(j == 0 ? "" : ",");
						append(sb, "@" + map.item(j).getNodeName());
						sb.append(":");
						append(sb, map.item(j).getNodeValue());
					}
					if (!hasChildNode(element)
							&& element.getTextContent() != null
							&& element.getTextContent().length() > 0) {
						sb.append(",");
						if (isCDATA) {
							String result = getCdataString(element
									.getTextContent());
							sb.append(result.substring(1, result.length() - 1));

							/*
							 * sb.append("{"); append(sb,"CDATA");
							 * sb.append(":");
							...
							sb.append(":");
							append(sb, element.getTextContent());
							// append(sb,element.getTextContent().replaceAll("\r",
							// "").replaceAll("\n", ""));
						}
					}
					if (hasChildNode(element)) {
