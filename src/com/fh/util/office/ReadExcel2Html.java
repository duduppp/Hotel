...
public class ReadExcel2Html {
	...
	public static void main(String[] args) {
		...
		String path = "G://公司文件//2017年6月工作计划表-杜平.xls";// E://Microsoft Excel
														// 工作表.xlsx
		InputStream is = null;
		...
		try {
			...
			if (wb instanceof XSSFWorkbook) {
				...
				htmlExcel = ReadExcel2Html.getExcelInfo(xWb, true);
			} else if (wb instanceof HSSFWorkbook) {
				HSSFWorkbook hWb = (HSSFWorkbook) wb;
				htmlExcel = ReadExcel2Html.getExcelInfo(hWb, true);
			}
			System.out.println(htmlExcel);
			 FileUtils.writeStringToFile(new File ("G://公司文件//", "杜平二月总结三月计划.html"), htmlExcel, "GB2312");
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			try {
	...
	public String readExcelToHtml(String filePath, boolean isWithStyle) {
		...
		try {
			...
			if (wb instanceof XSSFWorkbook) {
				XSSFWorkbook xWb = (XSSFWorkbook) wb;
				htmlExcel = ReadExcel2Html.getExcelInfo(xWb, isWithStyle);
			} else if (wb instanceof HSSFWorkbook) {
				HSSFWorkbook hWb = (HSSFWorkbook) wb;
				htmlExcel = ReadExcel2Html.getExcelInfo(hWb, isWithStyle);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private static void dealExcelStyle(Workbook wb, Sheet sheet, Cell cell,
		...
		if (cellStyle != null) {
			...
			if (wb instanceof XSSFWorkbook) {
				...
				int columnWidth = sheet.getColumnWidth(cell.getColumnIndex())/20;
				//得到宽度
				...
				int columnWidth = sheet.getColumnWidth(cellIndex)/20;
				sb.append("width:" + columnWidth + "px;");
