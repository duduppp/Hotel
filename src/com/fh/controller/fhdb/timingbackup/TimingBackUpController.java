...
public class TimingBackUpController extends BaseController {
	...
	public Object deleteAll() throws Exception {
		...
		if (Tools.notEmpty(DATA_IDS)) {
			...
			for (int i = 0; i < ArrayDATA_IDS.length; i++) {
				...
				this.removeJob(timingbackupService.findById(pd).getString(
						"JOBNAME")); // 删除任务
			}
	 ...
	 *            定时备份任务的ID
	 */
	public void addJob(String JOBNAME, String FHTIME, String TABLENAME,
			String TIMINGBACKUP_ID) {
		...
		QuartzManager.addJob(JOBNAME, JOB_GROUP_NAME, JOBNAME,
				TRIGGER_GROUP_NAME, DbBackupQuartzJob.class, FHTIME, parameter);
	}
