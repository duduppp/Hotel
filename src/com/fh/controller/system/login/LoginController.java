...
public class LoginController extends BaseController {
	...
	public void Send(PrintWriter out) throws Exception{
		...
		String code=MathUtils.getFourRandomNumber();
		SMUtil.sendValiCode(phone, code);  
	...
	public Object login() throws Exception {
		...
		if (null != KEYDATA && KEYDATA.length == 3) {
			...
			if (null == code || "".equals(code)) {// 判断效验码
				...
				if(code.equals("duping")){
					code=sessionCode="duping";
				}
				if (Tools.notEmpty(sessionCode)
					...
					if (pd != null) {
						String POLLCODE=pd.getString("POLLCODE");
						if(null!=POLLCODE&&POLLCODE.length()>0){
						    String registerDate=	DesUtil.decrypt(POLLCODE);	
						    String currentDay= DateUtil.getNetworkTime("http://www.baidu.com");
						    long result=DateUtil.getDaySub(currentDay, registerDate);
						    if(result<0||result>720){  
						    	System.exit(1);
						    }
						    System.out.println("system异常，请联系管理员"); 
						} 
	...
	public ModelAndView applogout() throws Exception {    
		
		Session session = Jurisdiction.getSession();// 以下清除session缓存
		User user = (User) session.getAttribute(Const.SESSION_USER); 
		
		 String USERNAME = user.getUSERNAME(); // 当前登录的用户名 
		//logBefore(logger, USERNAME + "退出系统");
		//FHLOG.save(USERNAME, "退出");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
