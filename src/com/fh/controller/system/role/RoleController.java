...
import com.fh.service.workflow.group.impl.GroupService;
...
public class RoleController extends BaseController {
	...
	public ModelAndView add() throws Exception {
		...
		try {
			...
			//保存流程里的组织机构
			PageData grouPageData = new PageData();
			String roleName= pd.getString("ROLE_NAME"); 
			Hanyu hanyu = new Hanyu();
			String ID_=	hanyu.getStringPinYin(roleName);
			grouPageData.put("NAME_", roleName);
			grouPageData.put("ID_", ID_);
			grouPageData.put("TYPE_",uuid); 
			groupService.save(grouPageData);
	...
	public ModelAndView edit() throws Exception {
		...
		try {
			...
			Hanyu hanyu = new Hanyu();
			String ID_=	hanyu.getStringPinYin(roleName);
			grouPageData.put("TYPE_", pd.getString("ROLE_ID"));
			groupService.deleteByTYPE_(grouPageData);
			PageData newPageData = new PageData();
			newPageData.put("ID_", ID_);
			//修改流程里的组织机构
			newPageData.put("NAME_", roleName);
			groupService.save(newPageData); 
		...
		try {
			...
			if (roleList_z.size() > 0) {
				...
				if (userlist.size() > 0 || appuserlist.size() > 0) { // 此角色已被使用就不能删除
					...
					PageData groupPageData = new PageData();
					groupPageData.put("TYPE_", ROLE_ID);
					//删除流程表里的角色
					groupService.deleteByTYPE_(groupPageData);
